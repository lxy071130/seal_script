...
class InfoManager {
    ...
    deleteMembers(team: string, members: string[]) {
        ...
        } else throw new Error("团队不存在或团队成员无效。");
    }
    addMembers(team: string, members: string[]) {
        if (this.getTeam(team) != false || members.length <= 0) {
            members.forEach((member) => {
                try {
